...
$(function () {
    ...
    wind.on("scroll", function () {
        ...
        if (bodyScroll > 100) {
            ...
            logo.attr('src', '/img/transha/logo.png');
        } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let isVisible = false;
    const topThreshold = 150; // Adjust this value as needed
    
    // Initial state - hidden
    contactContainer.style.transition = 'transform 0.4s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.4s ease';
    contactContainer.style.transform = 'translateY(120px)';
    contactContainer.style.opacity = '0';
    
    // Click handlers
    whatsappBtn.addEventListener('click', function() {
        window.open('https://wa.me/919605832333', '_blank');
    });
    
    phoneBtn.addEventListener('click', function() {
        window.open('tel:+919605832333', '_self');
    });
    
    function handleScroll() {
        const currentScrollY = window.scrollY;
        
        // Always hide at the very top of the page
    ...
    const navItems = document.querySelectorAll('.nav__item__container');

    navItems.forEach(item => {
        item.addEventListener('click', function() {
            // Remove active class from all items
            navItems.forEach(nav => nav.classList.remove('active__nav__item'));
            
            // Add active class to clicked item
            this.classList.add('active__nav__item');
